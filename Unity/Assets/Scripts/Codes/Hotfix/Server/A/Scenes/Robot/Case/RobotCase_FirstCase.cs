...
namespace ET.Server
{
    ...
    public class RobotCase_FirstCase: ARobotCase
    {
        protected override async ETTask Run(RobotCase robotCase)
        {
            ...
            {
                // M2C_TestRobotCaseALResponse response = await robotScene.GetComponent<Client.SessionComponent>().GateSession.Call(new C2M_TestRobotCaseALRequest() {N = robotScene.Zone}) as M2C_TestRobotCaseALResponse;
