...
namespace ET.Server
{
    ...
    public static class DBComponentSystem
    {
	    ...
	    public static async ETTask<T> QueryFirst<T>(this DBComponent self, Expression<Func<T, bool>> filter, string collection = null)
			    where T : Entity
	    {
	    ...
	    public static async ETTask Insert<T>(this DBComponent self, T data, string collection = null) where T: Entity
	    {
		    if (collection == null)
		    {
			    collection = typeof (T).Name;
		    ...
		    using (await CoroutineLockComponent.Instance.Wait(CoroutineLockType.DB, RandomGenerator.RandInt64() % DBComponent.TaskCount))
		    {
			    await self.GetCollection(collection).InsertOneAsync(data);
		    }
	    }
