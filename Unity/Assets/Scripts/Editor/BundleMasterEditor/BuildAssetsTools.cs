...
namespace BM
{
    public static class BuildAssetsTools
    {
        ...
        {
            PathHelper.CreateFile(Path.Combine(Application.dataPath, scriptFilePaths, "BPath.cs"));
            using (StreamWriter sw = new StreamWriter(Path.Combine(Application.dataPath, scriptFilePaths, "BPath.cs")))
            {
                ...
                sb.Append("{\n");
                sb.Append("\tpublic partial class BPath\n");
                sb.Append("\t{\n");
                foreach (string assetPath in allAssetPaths)
                {
                    string name = $"{Path.GetFileName(assetPath).Replace('.', '_')}";
