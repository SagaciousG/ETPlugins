...
namespace BM
{
    public partial class BundleMasterWindow
    {
        private void AssetsLoadSettingRender()
        {
            ...
            var buildName = EditorGUILayout.DelayedTextField(selectAssetsLoadSetting.BuildName, GUILayout.Width(_w / 8), GUILayout.ExpandWidth(false));
            ...
            EditorGUI.BeginChangeCheck();
            var bundleVariant = EditorGUILayout.TextField(selectAssetsLoadSetting.BundleVariant, GUILayout.Width(_w / 10), GUILayout.ExpandWidth(false));
            if (EditorGUI.EndChangeCheck())
            {
            ...
            if (!noLoadSetting)
            {
                ...
                for (int i = 0; i < assetPaths.Count; i++)
                {
                    GUILayout.BeginHorizontal();
                    EditorGUI.BeginChangeCheck();
                    var assetPath = EditorGUILayout.DelayedTextField(assetPaths[i], GUILayout.ExpandWidth(true));
                    if (EditorGUI.EndChangeCheck())
                    {
