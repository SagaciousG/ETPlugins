...
namespace BM
{
    ...
    {
        public static T[] LoadAllFromGroup<T>(string pathFolder, string bundlePackageName = null) where T : UnityEngine.Object
        {
            var list = new List<T>();
            ...
            if (!bundleRuntimeInfo.AllAssetLoadHandler.TryGetValue(pathFolder, out loadHandler))
            {
                loadHandler = LoadHandlerFactory.GetLoadHandler(pathFolder, bundlePackageName, true, true);
                bundleRuntimeInfo.AllAssetLoadHandler.Add(pathFolder, loadHandler);
                bundleRuntimeInfo.UnLoadHandler.Add(loadHandler.UniqueId, loadHandler);
            }
            loadHandler.Load();
            var allAssets = loadHandler.FileAssetBundle.LoadAllAssets<T>();
